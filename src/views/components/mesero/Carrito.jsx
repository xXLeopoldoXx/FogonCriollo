...
import { Flame, Link2, MessageSquare, Check, UtensilsCrossed } from 'lucide-react';
...
function ItemRow({ item, onQuitar, onEliminar, onNotaChange }) {
  ...
  return (
    <div className={styles.item}>
      ...
      >
        {item.nota ? <><MessageSquare size={12} /> {item.nota}</> : '+ Agregar nota'}
      </button>
    ...
    <div className={styles.root}>
      ...
      {ultimoPedidoId && (
        <div className={styles.linkBox}>
          <span className={styles.linkLabel}><Link2 size={14} /> Enlace para el cliente</span>
      ...
      <div className={styles.items}>
        {carrito.length === 0 ? (
          <div className={styles.emptyState}>
            <UtensilsCrossed size={32} className={styles.emptyIcon} />
      ...
      >
        {enviando ? 'Enviando...' : <><Flame size={15} /> Enviar a cocina</>}
      </Button>
