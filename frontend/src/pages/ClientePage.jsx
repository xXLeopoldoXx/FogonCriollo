// El Fogón Criollo — ClientePage v2

import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { io } from 'socket.io-client';
import { getSocket, EVENTS } from "../services/socketService";
import styles from './ClientePage.module.css';

const SOCKET_URL = window.location.origin; 

async function getClienteEspera() {
  const r = await fetch(`${import.meta.env.VITE_API_URL ?? '/api'}/pedidos/cliente/espera`);
  if (!r.ok) throw new Error('Error al conectar con la lista de espera');
  return r.json();
}

async function getClientePedido(id) {
  const r = await fetch(`${import.meta.env.VITE_API_URL ?? '/api'}/pedidos/${id}/cliente`);
  if (!r.ok) throw new Error('No se encontró el pedido solicitado');
  return r.json();
}
